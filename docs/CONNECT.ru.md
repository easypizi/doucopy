...
- ограничения респондера (write / read / shell). Skip = безопасный дефолт: запись только в workspace, shell выключен.
...
npx doucopy settings                   # запреты, фильтрация, модель (по harness), persona, harness
...
В репо-чекауте всё то же самое через `make chat`, `make status`, `make settings`, `make policy` и т.д. См. `make help`.

После `join` основной интерфейс — обычная фраза своему агенту. Скиллы сами зовут `ask_peer`.

### Ограничения и фильтрация

Три слоя на отвечающей машине:

1. **Permissions харнесса** (дефолт: запись только в workspace, shell off). Всегда закрыты для чтения `~/.ssh`, `~/.aws`, `~/.doucopy`. Править: `npx doucopy settings` (или `make settings` из чекаута). Пресеты модели зависят от harness: Cursor / Claude Code / Codex. Память подхватывает транскрипты Cursor, Claude Code и Codex, если их каталоги есть.
2. **`policy.md`** — мягкие правила тем и тона для LLM.
3. **`## Never reveal` / redact** — жёсткая пост-фильтрация ответа в коде демона.

У Codex только грубые режимы `--sandbox`, точечные пути там приблизительные.
